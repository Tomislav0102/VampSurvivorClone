...
public class Utils
{
    ...
    public static IEnumerator HitFlash(SpriteRenderer[] currentRenders)
    {
        for (int i = 0; i < currentRenders.Length; i++)
        {
            currentRenders[i].material = GameManager.Instance.spriteFlash;
        }
        yield return GetWait(0.07f);
        for (int i = 0; i < currentRenders.Length; i++)
        {
            currentRenders[i].material = GameManager.Instance.spriteDefault;
        }
    }
    ...
    public static void Activation<T>(T tip, GenActivation activation, string message = "")
    {
        ...
        switch (tip)
        {
            ...
            case SpriteRenderer spriteRenderer:
                if (spriteRenderer != null) spriteRenderer.enabled = activation == GenActivation.On;
                break;
        }
    }
    ...
    public static int IndexInGrid(Vector2Int gridSize, Vector2Int posInGrid)
    {
        return posInGrid.y * gridSize.x + posInGrid.x;
    }

    public static Vector2Int PosInGridFromIndex(Vector2Int gridSize, int index)
    {
    ...
    public static List<Ytutorial.Enemy> GetAllEnemiesInSpatialGroups(List<int> spatialGroups)
    {
        List<Ytutorial.Enemy> enemies = new List<Ytutorial.Enemy>();
